...
for key , names in students.items():
    if key in excercice and key in exams :
        meow = grader(int((excercice[key])) , int(exams[key])) 
        print(f"{students[key]:<30}{int(excercice[key]):<10}{int(excercice[key]//4):<10}{int(exams[key]):<10}{int(excercice[key])//4 + int(exams[key]):<10}{meow :<10}")
    else :
